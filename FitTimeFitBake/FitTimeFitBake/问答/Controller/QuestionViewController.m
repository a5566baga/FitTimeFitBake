...
-(void)initForMainView{
    ...
    [_dynamicButton setTitleColor:[UIColor redColor] forState:UIControlStateHighlighted];
    ...
    [_hotButton setTitleColor:[UIColor redColor] forState:UIControlStateHighlighted];
    ...
    [_noAnswerButton setTitleColor:[UIColor redColor] forState:UIControlStateHighlighted];
...
-(void)dynamicAction:(UIButton *)btn{
    if (btn.selected == NO) {
        ...
        [_questionView setSelectTypeStr:@"getNew"];
    }else{
        btn.selected = YES;
    }
}
-(void)hotAction:(UIButton *)btn{
    if (!btn.selected) {
        ...
        [_questionView setSelectTypeStr:@"getHot"];
    }else{
        btn.selected = YES;
    }
}
-(void)noAnswerAction:(UIButton *)btn{
    if (!btn.selected) {
        ...
        [_questionView setSelectTypeStr:@"getUnAnswer"];
    }else{
        btn.selected = YES;
    }
}
...
-(void)initForTableView{
    _questionView = [[QuestionView alloc] initWithFrame:CGRectMake(0, 64+HEIGHT, self.view.width, self.view.height-64-49-HEIGHT)];
